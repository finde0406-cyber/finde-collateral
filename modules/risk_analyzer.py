...
def analyze_dart_data(dart_data: dict) -> tuple:
    ...
    if financial:
        ...
        if equity is not None and debt is not None:
            ...
            if equity > 0:
                ...
                if debt_ratio >= 300:
                    ...
                    dart_summary['debt_ratio_ok'] = True

        # 영업이익 연속 적자 체크
        if len(financial) >= 2:
            loss_years = [f['year'] for f in financial if f.get('op_income') is not None and f['op_income'] < 0]
        ...
        if len(financial) >= 2:
            ...
            if rev_latest and rev_prev and rev_prev > 0:
                ...
                if rev_change <= -50:
                    violations.append(f"❌ 매출 급감 {rev_change:.1f}% ({financial[1]['year']}→{financial[0]['year']})")
                elif rev_change <= -30:
                    risk_factors.append(f"⚠️ 매출 감소 {rev_change:.1f}% ({financial[1]['year']}→{financial[0]['year']})")
    ...
    if risk_disclosures:
        ...
        for disc in risk_disclosures[:3]:  # 최대 3건만 violations에 추가
            risk_factors.append(f"⚠️ 위험공시: {disc['title']} ({disc['date'][:4]}.{disc['date'][4:6]}.{disc['date'][6:]})")

    return violations, risk_factors, dart_summary


def analyze_korean_stock(data, dart_data=None):
    """국내주식 리스크 분석 (DART 데이터 포함)"""
