    ...
    if not ticker:
        ...
        if is_korean:
            ...
            with st.spinner("조회 중..."):
                ...
                if data is None:
                    ...
                    st.warning("⏰ KRX 서버가 일시적으로 응답하지 않습니다. **30분~1시간 후** 재시도해주세요.")
                else:
                    # 기본 정보
                    st.markdown("### 📌 기본 정보")
                    ...
                    # 변동성
                    volatility = ((data['high_52w'] - data['low_52w']) / data['low_52w']) * 100 if data['low_52w'] > 0 else 0
                    ...
                    if violations:
                        ...
                        st.markdown("**의견**: 담보 인정 불가 (시총 100억 이상 종목으로 재신청)")
                    elif warnings:
                        st.warning("⚠️ **조건부 가능 (고위험)**")
                        st.markdown(f"**주의**: {', '.join(warnings)}")
                        st.markdown("**위험**: 🔴 급락 위험 매우 높음")
                        st.markdown("**조건**: 일일 모니터링 필수, 로스컷 130%")
                        st.markdown("**의견**: 변동성 극심. 로스컷 위험 고객 안내 필수")
                    else:
                        st.success("✅ **담보 가능**")
                        st.markdown("**위험**: 🟢 낮음")
        ...
        # === 해외주식 ===
            ...
            with st.spinner("조회 중..."):
                ...
                if 'error' in data:
                    st.error(f"❌ 조회 실패")
                    st.warning("⏰ Yahoo Finance API 제한에 걸렸습니다. **1시간 후** 재시도해주세요.")
                    
                    with st.expander("📋 임시 해결책"):
                        st.markdown("""
                        **현재 상황**: API 요청 제한 (Rate Limit)
                        
                        **해결 방법**:
                        1. **1시간 대기** 후 재시도 (가장 확실)
                        2. 다른 네트워크 사용 (모바일 핫스팟 등)
                        3. VPN 사용
                        
                        **예방**:
                        - 캐싱 기능 활용 (같은 종목 재조회 시 캐시 사용)
                        - 테스트는 몇 개 종목만
                        """)
                else:
                    # 기본 정보
